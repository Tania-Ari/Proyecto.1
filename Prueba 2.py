...
# Funcion que ocupamos en incisos siguientes (la movimos por comodidad)
...
if st.button("Descargar datos"):
    try:
        ...
        st.bar_chart(tabla_plot.set_index("Medida de Riesgo"))

        # INCISO f
        st.subheader("VaR con volatilidad móvil (Rolling 252)")
        ...
        var_95 = q_95 * rolling_std
        var_99 = q_99 * rolling_std
        ...
        st.subheader("Serie: Returns vs VaR (volatilidad móvil)")
        st.line_chart(df_vol)

        st.subheader("Violaciones (VaR volatilidad móvil)")

        viol_95 = (df_vol["Returns"] < df_vol["VaR_vol_95"]).sum()
        viol_99 = (df_vol["Returns"] < df_vol["VaR_vol_99"]).sum()

        total = len(df_vol)

        tabla_vol = pd.DataFrame({
            "Nivel": ["95%", "99%"],
            "Violaciones": [viol_95, viol_99],
            "Porcentaje (%)": [
                round(viol_95 / total * 100, 2),
                round(viol_99 / total * 100, 2)
            ]
        })

        st.table(tabla_vol)
        st.bar_chart(tabla_vol.set_index("Nivel"))
        ...
        ax.set_title("Histograma de rendimientos con distribución normal")
